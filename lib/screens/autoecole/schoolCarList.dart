...
import 'package:permis/screens/autoecole/schoolAddCar.dart';
import 'package:permis/screens/autoecole/schoolDrawerNavigator.dart';
...
class SchoolCarsList extends StatefulWidget {
  const SchoolCarsList({Key? key}) : super(key: key);

  @override
  State<SchoolCarsList> createState() => _SchoolCarsListState();
}

class _SchoolCarsListState extends State<SchoolCarsList> {
  bool loading = true;
  List<String> pic = [];
  List<String> brand = [];
  List<int> id = [];
  ...
  getCars() async {
    ...
    var datfa = await a.getUser(token);
    Map pp=json.decode(datfa.body);
    int idm=pp['id'];
    var bod = await a.getCarsSchool(token,idm);
